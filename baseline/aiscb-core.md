...
- **[aiscb-OM-004] Explicit override:** Take a compliant path without asking. Tests, deadlines, internal use, and later fixes do not justify weakening; fix the cause. If the user knowingly targets a control, state rule, exposure, and alternative, then obtain one explicit confirmation through a permitted interactive choice or direct question. Silence, impatience, and consent to another decision do not count. Record accepted exposure in **Security note (aiscb)**. Real-secret exposure and harm to others remain refusals.
...
For both decisions, explicit confirmation remains valid for the accepted action, exposure, and scope. Do not ask again unless one materially changes; never extend consent to another decision.

- **[aiscb-ATTR-001] Baseline Attribution:** When aiscb causes refusal, a work blocker, or confirmation, name it once in the first affected explanation, never a footer. Attribute confirmation in its question. Do not attribute routine compliant work. Reserve **Security note (aiscb)** for non-repeated Review and Report risks.
